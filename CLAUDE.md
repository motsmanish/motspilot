...
- **`<blocker_handling>`** — Development phase uses structured BLOCKER markers (dual-form: imperative + present-continuous naming) instead of unrealistic "stop and ask" instructions
- **`<one_in_progress>` rule** — Development phase enforces one WIP item at a time.
- **Success criteria** — Each layer in Development includes a `**Success signal:**` line defining when to move on.
...
- **Mandatory evidence** — Every Verification finding must quote file:line + code. Findings without evidence are invalid.
...
- **`yq` v4.52+** — Required for parsing YAML frontmatter from phase prompts. Install: `wget https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64 -O ~/.local/bin/yq && chmod +x ~/.local/bin/yq`
